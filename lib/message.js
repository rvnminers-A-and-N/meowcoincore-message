...
var ECDSA = ritocore.crypto.ECDSA;
var Signature = ritocore.crypto.Signature;
var sha256sha256 = ritocore.crypto.Hash.sha256sha256;
...
/**
 * Will sign a message with a given ritocoin private key.
 *
...
/**
 * Will return a boolean of the signature is valid for a given ritocoin address.
 * If it isn't the specific reason is accessible via the "error" member.
 *
 * @param {Address|String} ritocoinAddress - A ritocoin address
 * @param {String} signatureString - A base64 encoded compact signature
 * @returns {Boolean}
 */
Message.prototype.verify = function verify(ritocoinAddress, signatureString) {
  $.checkArgument(ritocoinAddress);
  $.checkArgument(signatureString && _.isString(signatureString));

  if (_.isString(ritocoinAddress)) {
    ritocoinAddress = Address.fromString(ritocoinAddress);
  }
  ...
  var signatureAddress = Address.fromPublicKey(publicKey, ritocoinAddress.network);

  // check that the recovered address and specified address match
  if (ritocoinAddress.toString() !== signatureAddress.toString()) {
