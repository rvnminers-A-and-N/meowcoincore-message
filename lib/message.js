...
var ECDSA = meowcoincore.crypto.ECDSA;
var Signature = meowcoincore.crypto.Signature;
var sha256sha256 = meowcoincore.crypto.Hash.sha256sha256;
...
Message.MAGIC_BYTES = new Buffer('Rito Signed Message:\n');
...
/**
 * Will sign a message with a given meowcoin private key.
 *
...
/**
 * Will return a boolean of the signature is valid for a given meowcoin address.
 * If it isn't the specific reason is accessible via the "error" member.
 *
 * @param {Address|String} meowcoinAddress - A meowcoin address
 * @param {String} signatureString - A base64 encoded compact signature
 * @returns {Boolean}
 */
Message.prototype.verify = function verify(meowcoinAddress, signatureString) {
  $.checkArgument(meowcoinAddress);
  $.checkArgument(signatureString && _.isString(signatureString));

  if (_.isString(meowcoinAddress)) {
    meowcoinAddress = Address.fromString(meowcoinAddress);
  }
  ...
  var signatureAddress = Address.fromPublicKey(publicKey, meowcoinAddress.network);

  // check that the recovered address and specified address match
  if (meowcoinAddress.toString() !== signatureAddress.toString()) {
